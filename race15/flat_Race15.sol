...
/** 
 *  SourceUnit: /Users/danielguerrero/workspace/INFOSEC/secureum-races/race15/Race15.sol
*/
...
contract SimpleDEX {
    ...
    constructor(uint first_balance, address _token, address _owner) payable {
        ...
        token = IERC20(_token);
        bool token_success = token.transferFrom(
            msg.sender,
            address(this),
            first_balance
        );
...
contract SimpleDexProxy {
    function buyEth(address simpleDexAddr, uint amount) external {
        ...
        (bool success, ) = (simpleDexAddr).call(
            abi.encodeWithSignature("buyEth(uint)", amount)
        );
