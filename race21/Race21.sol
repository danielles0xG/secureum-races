...
A.
Minting does not take place unless precisely ADOPTION_PRICE is paid in lidoToken
B.
lidoToken is a known token and is immutable in this contract, therefore transferFrom is safe to use
C.
In some but not all cases the burry() function can cause the adopt() function to be DOS’d
D.
Duplicate bunnies array entries will be produced if they burn their Rabbits NFT using the Rabbits ERC721 contract directly instead of using the RabidRabbits.burry() function