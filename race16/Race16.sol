...
contract FlashLoan is IERC3156FlashLender {
    ...
    function flashLoan(
        ...
        uint256 oldAllowance = IERC20(token).allowance(
            address(this),
            address(receiver)
        );
        ...
        if (newBal < oldBal + fee) {
            ...
            require(
                IERC20(token).transferFrom(msg.sender, address(this), retAmt),
                "All funds not returned"
            );
        }

        if (
            IERC20(token).allowance(address(this), address(receiver)) >
            oldAllowance
        ) {
